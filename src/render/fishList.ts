...
export const renderFishList = (root: HTMLElement, fish: readonly TankFish[], status: StockStatus | null, onRemove: (uid: string) => void) => {
  ...
  fish.forEach(item => { const row = document.createElement("article"); row.className = "fish"; const name = document.createElement("div"); name.className = "fish-name"; name.textContent = `${item.emoji} ${item.name}`; const meta = document.createElement("span"); meta.className = "fish-meta"; meta.textContent = `${item.inchSize.toFixed(1)} inch`; name.append(meta); const button = document.createElement("button"); button.className = "remove"; button.type = "button"; button.textContent = "×"; button.setAttribute("aria-label", `Remove ${item.name}`); button.addEventListener("click", () => onRemove(item.uid)); row.append(name, button); list.append(row) })
}
