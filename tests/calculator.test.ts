...
test("calculator rejects non-finite values", () => { assert.equal(getRatio(Number.NaN, 10), null); assert.equal(getHeadroom(Number.POSITIVE_INFINITY, 10), null); assert.equal(getStockStatus(Number.NaN), null) })
