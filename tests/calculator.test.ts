...
import { getHeadroom, getRatio, getStockStatus, getTotalInches } from "../src/calculator.js"

const fish = [{ inchSize: 1.5 }, { inchSize: 2 }] as never[]
test("calculator handles totals, ratio, headroom", () => { assert.equal(getTotalInches(fish), 3.5); assert.equal(getRatio(3.5, 10), .35); assert.equal(getHeadroom(3.5, 10), 6.5) })
